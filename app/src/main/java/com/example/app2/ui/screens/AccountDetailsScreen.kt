...
fun AccountDetailsScreen(
    ...
    Scaffold(
        ...
        Column(Modifier.padding(padd).padding(16.dp)) {
            ...
            Row {
                Button(modifier = Modifier.testTag("detailsEdit"),onClick = onEdit) { Text("Редактировать") }
                Spacer(Modifier.width(12.dp))
                OutlinedButton(modifier = Modifier.testTag("detailsDelete"),onClick = { vm.delete(number); onBack() }) { Text("Удалить") }
            }
        }
    }
}
